...
export default function Record() {
  ...
  useEffect(() => {
    if (userPosts.length > 0) {
      const posts = userPosts.filter(
        (post) => post.date !== undefined && post.date < Number(nextMonday)
      );
  ...
  const handleSeletedDegree = (idx: number) => {
    const posts = userRecords.filter(
      (post) =>
        post.degree &&
        post.degree >= degrees[idx] &&
        post.degree < degrees[idx + 1]
    );
    setPosts(posts);
  };
    ...
    <div className="flex min-h-[calc(100vh-3.3rem)] pt-16 bg-base-200 px-4">
      {posts.length === 0 ? (
        <div className="card gap-5 my-8 mx-auto w-full min-w-[350px] max-w-[1024px] bg-base-100 shadow-xl p-7">
          <h2 className="text-2xl sm:text-4xl font-extrabold text-center pt-5 pb-5">
            Record
          </h2>
          ...
          <div className="form-control w-full">
            <div className="input-group input-group-xs justify-end">
              <select
                ...
                onChange={(e) =>
                  handleSeletedDegree(e.target.selectedIndex - 1)
                }
                defaultValue={"Select degree Range"}
              >
          ...
          <div className="grid xs:grid-cols-2 gap-2 md:gap-6 justify-center justify-items-center">
            {posts.map((post, idx) => {
              ...
              return (
                <ImageFrame
                  key={idx}
                  content={`${timeStamp.getFullYear()}년 ${
                    timeStamp.getMonth() + 1
                  }월 ${timeStamp.getDate()}일 ${days[timeStamp.getDay()]}요일${
